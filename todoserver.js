...
app.use("/uploads",express.static(path.join(__dirname,"uploads"))) // Serve static files from the "assign" folder
...
app.post("/todo", function (req, res) {
  ...
  const picture = req.file; // Use req.file to get the uploaded picture data
  console.log(todoData);
  console.log(picture);
  saveTodoInFile(todoData, email, picture, function (err) {
    if (err) {
      console.error(err);
...
function saveTodoInFile(todoData, email, picture, callback) {
  readtaskFromFile(function (err, data) {
    if (err) {
      callback(err);
      return;
    ...
    const todo = {
      ...
      id,
      completed,
      picture: picture ? picture.path : null, // Save the path of the uploaded picture in the todo object
    };
